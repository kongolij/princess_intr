...
//	API PATH: https://api.bigcommerce.com/stores/w1jeucyusb/v3/
	...
	public static HttpURLConnection createRequest(String storeHash, String accessToken, String endpoint, String method,
		...
		String urlWithParams = queryParams != null && !queryParams.isEmpty() ? baseUrl + "?" + formatParams(queryParams)
				: baseUrl;
	...
	public static HttpURLConnection createGraphQlRequestForGraphQL(String storeHash, String channelId,
			String storefrontToken) throws Exception {
		String urlStr = "https://store-" + storeHash + "-" + channelId + ".mybigcommerce.com/graphql";
		URL url = new URL(urlStr);
		HttpURLConnection connection = (HttpURLConnection) url.openConnection();

		connection.setRequestMethod("POST");
		connection.setRequestProperty("Authorization", "Bearer " + storefrontToken);
		connection.setRequestProperty("Content-Type", "application/json");
		connection.setDoOutput(true);
	...
	private static String formatParams(Map<String, Object> queryParams) {
		return queryParams.entrySet().stream().map(entry -> {
			...
			if (entry.getValue() instanceof List) {
				...
				value = ((List<?>) entry.getValue()).stream().map(item -> encode(item.toString()))
