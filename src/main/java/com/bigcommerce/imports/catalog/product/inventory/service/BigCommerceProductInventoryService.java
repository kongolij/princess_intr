...
public class BigCommerceProductInventoryService {
	...
	public void updateVariantInventory(List<InventoryRecord> inventoryRecords) throws Exception   {
		...
		JSONArray inventoryAdjustmentItems  = BigCommerceInventoryMapper.buildInventoryAdjustmentPayload(inventoryRecords,
				skuToIdsMap, storeLocationMap);
		...
		for (int i = 0; i < items.size(); i += BATCH_SIZE) {
		    ...
		    try {
		        ...
		        System.out.printf("✅ Inventory batch [%d - %d) updated successfully.%n", i, Math.min(i + BATCH_SIZE, items.size()));
		    } catch (Exception e) {
		        System.err.printf("❌ Failed inventory update for batch [%d - %d): %s%n",
		                i, Math.min(i + BATCH_SIZE, items.size()), e.getMessage());
		        e.printStackTrace();
		    }
		}
